...
class SinglyLCMF(val problem: Problem, val scheme: SimpleScheme, val tol: Double = 1e-9, maxIterations: Int = 1000) extends MargParI {
  ...
  private val cWeights: mutable.Map[Int,Factor] = new mutable.HashMap[Int,Factor]()
    .withDefault(variable => Factor.maxEntropy(Array(variable),problem.domains,problem.ring))
  /**
   * The marginals of the variables under conditions;
   * together with `cWeights`, these define the approximate distribution.
   */
  private val conditionedQs: mutable.Map[(Int,Condition),Factor] =
    new mutable.HashMap[(Int,Condition),Factor]().withDefault((createInitialMarginal _).tupled)
  ...
  /** The logarithmic factors of the problem adjacent to each variable. This is just a cache for
    * faster computation. */
  val logFactors: Map[Int,IndexedSeq[Factor]] = {
    def logFactorsOf(v: Int): IndexedSeq[Factor] =
  ...
  def createInitialMarginal(variable: Int, condition: Condition): Factor = {
    require(condition.keySet == scheme.influencesOf(variable))
    Factor.deterministicMaxEntropy(Array(variable), condition, problem.domains, problem.ring)
  }
  ...
  def estimatedDistribution(scope: Array[Int], condition: Condition = Map()): Factor = {
    ...
    def getMarginal(variable: Int, condition: Condition): Factor = {
    ...
    val marginals: IndexedSeq[Factor] = scope.map(getMarginal(_,condition))
    val conditionEnforcer: Factor = Factor.deterministicMaxEntropy(
      condition.keys.filter(scope.contains).toArray,
      condition,
      problem.domains,
      NormalD)
    Factor.multiply(NormalD)(problem.domains)(marginals :+ conditionEnforcer)
  }

  /** Recalculate the distribution over the conditions induced by the given variable. */
  def computeNewConditionWeights(variable: Int): Factor = {
    ...
    Factor.fromFunction(Array(variable),problem.domains, {
  ...
  def computeNewMarginal(variable: Int, condition: Condition): Factor = {
    require(!condition.contains(variable))

    val factorContribs: IndexedSeq[Factor] = for {
      logFactor <- logFactors(variable)
      qDist = estimatedDistribution(logFactor.variables.filterNot(_ == variable),condition)
      expectedLog = Factor.multiplyRetain(NormalD)(problem.domains)(Array(logFactor, qDist), Array(variable))
    } yield expectedLog
    Factor.elementWiseSum(factorContribs).map(math.exp).normalize(NormalD)
  }
  ...
  private def updateConditionWeights(variable: Int): Boolean = {
    ...
    val diff = Factor.maxDiff(cWeights(variable),newWeights,NormalD)
    if(diff > tol){
  ...
  private def updateQ(variable: Int, condition: Condition): Boolean = {
    ...
    val diff = Factor.maxDiff(conditionedQs(variable -> condition),newDist,NormalD)
    if(diff > tol){
  ...
  def logExpectation(f: Factor, cond: Condition = Map()): Double = {

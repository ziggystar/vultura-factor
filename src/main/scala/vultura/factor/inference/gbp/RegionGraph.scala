...
case class Region(cr: Double, variables: Set[Int], factors: Set[Factor]){
...
object Region{
  def fromFactors(fs: Iterable[Factor], cr: Double = 1): Region = {
    val fSet = fs.toSet
...
trait RegionGraph {
  ...
  def factorCount(f: Factor): Double
  /** @return true if subgraph induced by containing given factor is connected. */
  def factorConnectedness(f: Factor): Boolean
...
case class MapRegionGraph(problem: Problem, regions: Set[Region], edges: Set[(Region,Region)]) extends RegionGraph {
  ...
  def factorCount(f: Factor): Double = regions.filter(_.factors.contains(f)).toSeq.map(_.cr).sum
  /** @return true if subgraph induced by containing given factor is connected. */
  def factorConnectedness(f: Factor): Boolean =
...
object RegionGraph{
  ...
  def betheRG(problem: Problem): RegionGraph = {
    ...
    val factorRegions: Map[Factor, Region] = (for{
      f <- problem.factors
  ...
  def saturatedRG(problem: Problem, _initialRegions: Iterable[(Set[Int],Iterable[Factor])]): RegionGraph = {
  ...
  def saturatedRG(problem: Problem, random: Random): RegionGraph = {
    ...
    val factorRegions: Map[Set[Int], IndexedSeq[Factor]] = problem.factors.groupBy(f => regionsSSet.superSetsOf(f.variables.toSet).pickRandom(random))

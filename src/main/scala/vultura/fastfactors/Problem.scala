...
case class Problem(factors: IndexedSeq[FastFactor], domains: Array[Int], ring: RingZ[Double]) extends BasicProblem {
  ...
  lazy val factorsOfVariable: Array[Array[FastFactor]] = {
    val result: Array[Array[FastFactor]] = degrees.map(new Array[FastFactor](_))
  ...
  def condition(condition: Map[Var,Val]): Problem = {
    ...
    newP.copy(factors = newP.factors ++ condition.map(kv => FastFactor.deterministicMaxEntropy(Array(kv._1),Map(kv),domains,ring)))
  }

...
case class Factor(variables: Array[Int], values: Array[Double]) {
  ...
  def simplify(domains: Array[Int]): Factor = {
    def isIndependentIn(factor: Factor, variable: Int): Boolean = {
      ...
      Factor.sumProduct(remVars,domains,Array(factor.variables),Array(factor.values): Array[Array[Double]],Factor.AdditionIsEquality,resultArray)
  ...
  def condition(condition: Map[Int,Int], domains: Array[Int]): Factor = {
    ...
    if(hitVars.isEmpty)
      ...
      val lookup: Array[Int] = Factor.buildLookup(remVars, domains, this.variables)
...
object Factor{
  ...
  def elementWiseSum(factors: IndexedSeq[Factor]): Factor = {
    ...
    Factor(scope,Array.tabulate(factors.head.values.size)(i => factors.foldLeft(0d)(_ + _.values(i))))
  }

  object AdditionIsEquality extends Ring[Double]{
  ...
  def maxEntropy(variables: Array[Int], domains: Array[Int], ring: Ring[Double]): Factor =
  ...
  def multiplyRetain(ring: Ring[Double])(domains: Array[Int])(factors: Seq[Factor], retain: Array[Int]): Factor = {
  ...
  def kl(f1: Factor, f2: Factor, ring: Ring[Double]): Double = {
  ...
  def maxDiff(f1: Factor, f2: Factor, ring: Ring[Double]): Double = {

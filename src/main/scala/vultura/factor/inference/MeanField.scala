 ...
 */
class MeanField(val problem: Problem, val tol: Double = 1e-9, val maxIter: Int = 10000) extends MargParI  {
  ...
  def createQFactor(variables: Array[Int]): Factor = Factor.multiply(NormalD)(problem.domains)(variables.map(q))
  ...
  def updateVariable(v: Int): Boolean = {
    val newDist: Factor = computeMarginal(v)
    val diff = Factor.maxDiff(q(v),newDist,NormalD)
    if(diff > tol){
      q.put(v,newDist)
      return true
    }
  ...
  def computeMarginal(v: Int): Factor = {
    val factorContribs: IndexedSeq[Factor] = for {
      logFactor <- logFactors(v)
      qdist = createQFactor(logFactor.variables.filterNot(_ == v))
      expectedLog = Factor.multiplyRetain(NormalD)(problem.domains)(Array(logFactor, qdist), Array(v))
    } yield expectedLog
    val newDist: Factor = Factor.elementWiseSum(factorContribs).map(math.exp).normalize(NormalD)
    newDist
  }
  ...
  def variableBelief(vi: Int): Factor = q(vi).copy()

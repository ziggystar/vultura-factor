...
import vultura.fastfactors.FastFactorMatchers
...
class BP_CalTest extends Specification with FastFactorMatchers{
  ...
  override def is: Fragments =
    "must be exact on tree problem" ! {
      new BP_Cal(treeProb).logZ must beCloseTo(new CalibratedJunctionTree(treeProb).logZ, 0.01)
    }
}

  ...
  def is =
    ...
    "building factors" ^
      "from function, not using all variables" !
        (Factor.fromFunction(Array(0),Array(2,2),_ => 1d) === Factor(Array(0),Array(1d,1d))) ^
      "deterministic max entropy factors" ^
      "with empty condition" !
        (Factor.deterministicMaxEntropy(Array(0),Map(),Array(2),NormalD) must haveValuesCloseTo(FF(VARS(0), VALS(0.5,0.5)))) ^
      "irrelevant entries in condition shouldn't matter" !
        (Factor.deterministicMaxEntropy(Array(0),Map(1 -> 1),Array(2),NormalD) must haveValuesCloseTo(FF(VARS(0), VALS(0.5,0.5)))) ^
      "complete condition, one var" !
        (Factor.deterministicMaxEntropy(Array(0),Map(0 -> 0),Array(2),NormalD) must haveValuesCloseTo(FF(VARS(0), VALS(1,0)))) ^
      "complete condition, two vars" !
        (Factor.deterministicMaxEntropy(Array(0,1),Map(0 -> 0,1 -> 0),Array(2,2),NormalD) must haveValuesCloseTo(FF(VARS(0,1), VALS(1,0,0,0)))) ^
      "partial condition" !
        (Factor.deterministicMaxEntropy(Array(0,1),Map(0 -> 0),Array(2,2),NormalD) must haveValuesCloseTo(FF(VARS(0), VALS(0.5,0,0.5,0)))) ^
      "some condition" !
        (Factor.deterministicMaxEntropy(Array(1),Map(1->0),Array(2,2,2),NormalD) must haveValuesCloseTo(FF(VARS(1),VALS(1,0)))) ^
      p^
    "eval method" ^
      (Factor.fromFunction(Array(0,1,2),Array(2,3,4), _.sum).eval(Array(1,2,3), domains = Array(2,3,4)) === 6) ^
      (Factor.fromFunction(Array(0,1,2),Array(2,3,4), _.sum).eval(Array(0,1,1), domains = Array(2,3,4)) === 2) ^
      (Factor.fromFunction(Array(0,1,2),Array(2,3,4), _.sum).eval(Array(1,1,1), domains = Array(2,3,4)) === 3)

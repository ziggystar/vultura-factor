...
class LCBP(val problem: Problem,
  ...
  trait FactorEdge extends HashMemo { self: CEdge with Product =>
    ...
    override def create: TOut = Factor.maxEntropy(variables,problem.domains,problem.ring)
  ...
  case class V2F(v: Int, f: Factor, vc: Condition) extends CEdge with FactorEdge {
  ...
  case class FactorBelief(f: Factor, fc: Condition) extends CEdge with FactorEdge {
  ...
  case class F2V(f: Factor, v: Int, fc: Condition) extends CEdge with FactorEdge {
    override type ETIn = V2F
    def variables = Array(v)
  ...
  /** Message from a factor to a variable after summing over the refined conditions of the factor.
    ...
    * @param vc Variable Condition.
    */
  case class F2VSummed(f: Factor, v: Int, vc: Condition) extends CEdge with FactorEdge {
    type ETIn = CEdge
    def variables = Array(v)
    ...
    override def compute: (IndexedSeq[TIn]) => TOut = in => {
      ...
      val rest  = in.view.tail.map(_.asInstanceOf[Factor])

...
trait LCBPAlg[X] {
  def inferWithScheme(p: Problem, s: FactoredScheme, tol: Double = 1e-12, maxIter: Int = 10000): (MargParI, Boolean, X)
}

/** The old implementation, using exact meta inference with counting. */
object OldLCBP extends LCBPAlg[LCBP] {
  override def inferWithScheme(p: Problem, s: FactoredScheme, tol: Double = 1e-12, maxIter: Int = 10000): (MargParI, Boolean, LCBP) = {
    val lcbp = new LCBP(p,s.toGScheme,tol,maxIter)
...
object LCBP_G_Exact extends LCBPAlg[LCBPGeneral] {
  override def inferWithScheme(p: Problem, s: FactoredScheme, tol: Double, maxIter: Int): (MargParI, Boolean, LCBPGeneral) = {
...
object LCBP_BP extends LCBPAlg[LcbpMetaBP] {
  override def inferWithScheme(p: Problem, s: FactoredScheme, tol: Double, maxIter: Int): (MargParI, Boolean, LcbpMetaBP) = {

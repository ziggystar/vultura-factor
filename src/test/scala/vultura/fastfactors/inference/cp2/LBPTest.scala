...
class LBPTest extends Specification {
  ...
  def increment1_p1 = new MutableFIFOCalibrator[lbp.BPMessage](MaxDiff, problem = lbp.cp, initialize = c_p1)
  ...
  override def is: Fragments = {
    ...
    "cp2.LBP must infer same logZ as old LBP" ! (LBP.infer(p1).logZ must beCloseTo(new BeliefPropagation(p1, tol = 1e-10, runInitially = 10000).logZ, 1e-5))
  }
}

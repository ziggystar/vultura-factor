...
object SequentialImportanceSampling {
  ...
  def weightedImportanceSampling[A](factorSequence: Seq[Seq[A]], numSamples: Int, ring: RingWithZero[Double], random: Random)(implicit ev: Factor[A,Double]): ParticleSeq[Double] = {
    ...
    val particles = factorSequence.tail.foldLeft(initialParticles){ case(particles,newFactors) =>
      ...
      val unweightedParticles: Seq[(WrappedArray[Int], Double)] = for(
        (oldAssignment,oldWeights) <- particles.particles.toSeq;
        oldWeight <- oldWeights
      ) yield {
        val conditionedNewFactor: ProductFactor[A, Double] = newFactor.condition(particles.variables,oldAssignment.toArray)
      ...
      ParticleSeq(
        (particles.variables ++ variables(newFactor)).distinct,
        (particles.domains ++ domains(newFactor)).distinct,

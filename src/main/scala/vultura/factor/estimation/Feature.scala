...
case class Feature(variables: Array[Var], point: Array[Val]) {
   ...
   */
  def toFastFactor(domains: Array[Int], ring: Ring[Double], theta: Double) =
    Factor(variables,Array.fill(variables.map(domains).product)(ring.one)).set(point,domains,theta)
...
object Feature {
  ...
  def buildProblem(domains: Array[Int], ring: Ring[Double], weightedFeatures: Iterable[(Feature,Double)]): Problem =
  ...
  def streamLineProblem(p: Problem): Problem = {
    ...
    def rescale(f: Factor): (Factor, Double) = {
    ...
    p.copy(factors = newFactors :+ Factor(Array(),Array(p.ring.prodA(multipliers.toArray))))
  }

...
class CalibratedJunctionTreeTest extends Specification with FastFactorMatchers{
  ...
  def testSampleMarginals(_p: Problem, numSamples: Int = 1000, tol: Double = 1e-2) = {
    val p = _p.toRing(NormalD)
    ...
    p.factors.map(f => empiricalMarginal(f.variables) must beSimilarTo(jt.cliqueBelief(f.variables), tol)).reduce(_ and _)
  }
}

...
package object generation {
  ...
  /** Works only for binary valued problems.
    * For each variable, the energy generated by `energyOfOne` is added if the variable is 0. */
  def withMagneticField[A](p: LabeledProblem[A], energyOfOne: Generator[Double], random: Random): LabeledProblem[A] = {
    require(p.problem.domains.forall(_ == 2))
    require(p.problem.ring == LogD)
    val singletons = p.problem.variables.map(v => Factor(Array(v), Array(energyOfOne.generate(random), LogD.one)))
    p.copy(problem = p.problem.copy(factors = p.problem.factors ++ singletons))
  }

  def generateGrid(dimensions: IndexedSeq[(Int,Boolean)], domainSize: Int = 2): LabeledProblemStructure[IndexedSeq[Int]] =
    StructureGenerator.fixedDomainSize[IndexedSeq[Int]](Constant(GraphGenerator.lattice(dimensions:_*)), domainSize).generate(null)
}

...
trait CalibrationProblem[A] {
  ...
  def parents: Map[A, Set[(Either[Factor,A],Double)]]
  def nodes: Set[A]
...
object CalibrationProblem {
  ...
  def normalizeNodes[A](cp: CalibrationProblem[A]): (CalibrationProblem[Int], Map[A,Int]) = {
    ...
    val p = new CalibrationProblem[Int]{
      ...
      /** Map each node to a set of parents. Each parent has a weight and is either another node or a constant factor. */
      def parents: Map[Int, Set[(Either[Factor, Int], Double)]] = cp.parents.map{
        case (a,preds) => map(a) -> preds.map{
          case (Right(parent),x) => (Right(map(parent)): Either[Factor,Int]) -> x
  ...
  case class FactorN(f: Factor) extends BetheNode
  case class VariableN(v: Int) extends BetheNode
  case class F2V(f: Factor,v: Int) extends BetheNode
  case class V2F(v: Int, f: Factor) extends BetheNode
...
class RoundRobinCalibrator[A](_graph: CalibrationProblem[A], maxSteps: Int = 10, tol: Double = 1e-10, random: Random = new Random(0)){
  ...
  private val messages: IntMap[Factor] = {
    def createFactor(n: Int): Factor = {
      Factor.maxEntropy(graph.nodeScope(n).toArray.sorted, graph.problem.domains, graph.problem.ring)
    }
  ...
  private def updateNode(n: Int): Unit = {
    ...
    val result = Factor.multiplyRetain(ring)(domains)(incoming,messages(n).variables).normalize(ring)
    System.arraycopy(result.values,0,messages(n).values,0,result.values.size)
  }

  def resultOf(node: A): Factor = messages(nodeMap(node))
}
